<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="Admin Action">
    ...
    <a href="{{ route('admin.milestone.settings.edit',$new->uuid) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <form id="delete-data-{{ $new->uuid }}" action="{{ route('admin.milestone.settings.destroy', $new->uuid) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
