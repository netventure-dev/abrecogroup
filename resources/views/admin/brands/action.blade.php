<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="Admin Action">
    {{-- @can('update', $admin) --}}
    <a href="{{ route('admin.brands.edit',$model->id) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <form id="delete-data-{{ $model->id }}" action="{{ route('admin.brands.destroy', $model->id) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
