<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="Admin Action">
    {{-- @can('update', $admin) --}}
    <a href="{{ route('admin.owner.edit',$owner->id) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <button type="button" class="last btn btn-primary delete" data-toggle="tooltip" title="Delete" onclick="event.preventDefault(); if(confirm('{{__('Are you sure to delete this row')}}')){
        document.getElementById('delete-data-{{ $owner->id }}').submit();}">
        <i class="fas fa-trash-alt"></i>
    </button>
    <form id="delete-data-{{ $owner->id }}" action="{{ route('admin.owner.destroy', $owner->id) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
