<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="Admin Action">
    {{-- @can('update', $admin) --}}
    <a href="{{ route('admin.news.edit',$blog->uuid) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <form id="delete-data-{{ $blog->uuid }}" action="{{ route('admin.news.destroy', $blog->uuid) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
