<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="Admin Action">
    {{-- @can('update', $admin) --}}
    <a href="{{ route('admin.kms.edit',$admin->id) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <form id="delete-data-{{ $admin->id }}" action="{{ route('admin.kms.destroy', $admin->id) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
