...
@section('title') {{ __('Case Study Contents') }} @endsection
        ...
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                ...
                <div class="page-title-right">
                         <a href="{{ route('admin.casestudies.contents.create',@$casestudy->uuid) }}"
                            class="btn btn-primary w-md">{{ __('Create Case Study Contents') }}</a> 
                </div>
            </div>
        </div>
    </div>
                    ...
                    <table class="table table-bordered table-striped yajra-datatable" style="    background: #b2b2b2;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{!! $content->title !!}</td>
                                <td>{{$content->order}}</td>
                                <td> <a href="{{ route('admin.casestudies.contents.edit',['id' => $casestudy->uuid, 'uuid' =>$content->uuid]) }}" class="first btn btn-primary edit"> Edit</a>
                                ...
                                <form id="delete-data-{{ $content->uuid }}" action="{{ route('admin.casestudies.contents.destroy',['id' => $casestudy->uuid, 'uuid' =>$content->uuid]) }}"
                                    method="POST">
                                    @csrf
                                </form>
                            </td>
