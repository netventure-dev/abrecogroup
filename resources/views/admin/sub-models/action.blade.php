<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="Admin Action">
    {{-- @can('update', $admin) --}}
    <a href="{{ route('admin.sub-models.edit',$model->id) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <form id="delete-data-{{ $model->id }}" action="{{ route('admin.sub-models.destroy', $model->id) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
