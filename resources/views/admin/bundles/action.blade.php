<div class="mt-2 btn-group btn-group-sm" role="group" aria-label="rod Action">
    {{-- @can('update', $size) --}}
    <a href="{{ route('admin.bundles.edit',$bundle->id) }}" class="first btn btn-primary edit"><i class="fas fa-pencil-alt"
    ...
    <button type="button" class="last btn btn-primary delete" data-toggle="tooltip" title="Delete" onclick="event.preventDefault(); if(confirm('{{__('Are you sure to delete this row')}}')){
        document.getElementById('delete-data-{{ $bundle->id }}').submit();}">
        <i class="fas fa-trash-alt"></i>
    </button>
    <form id="delete-data-{{ $bundle->id }}" action="{{ route('admin.bundles.destroy', $bundle->id) }}"
        method="POST">
        @csrf
        @method('DELETE')
    </form>
