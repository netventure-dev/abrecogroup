<?php

namespace App\DataTables\Admin;

use App\Models\SubModel;
use Yajra\DataTables\Html\Button;
use Yajra\DataTables\Html\Column;
use Yajra\DataTables\Html\Editor\Editor;
use Yajra\DataTables\Html\Editor\Fields;
use Yajra\DataTables\Services\DataTable;

class SubModelDataTable extends DataTable
{
    /**
     * Build DataTable class.
     *
     * @param mixed $query Results from query() method.
     * @return \Yajra\DataTables\DataTableAbstract
     */
    public function dataTable($query)
    {
        return datatables()
        ->eloquent($query)
        ->addIndexColumn()
        ->editColumn('name', function (SubModel $model) {
            return $model->name;
        })
        ->editColumn('difficulty_id', function (SubModel $model) {
            if(isset($model->sale_difficulty)){
                return $model->sale_difficulty->name;

            }
        })
        ->editColumn('brand_id', function (SubModel $model) {
            if(isset($model->brand_id)){
                return $model->brand_data->name;

            }
        })
        ->editColumn('status', function (SubModel $model) {
            if ($model->status) {
                return '<span class="btn btn-sm btn-success btn-rounded waves-effect waves-light">Active</span>';
            } else {
                return '<span class="btn btn-sm btn-danger btn-rounded waves-effect waves-light">Inactive</span>';
            }
        })
        ->addColumn('action', function (SubModel $model) {
            return view('admin.sub-models.action', compact('model'));
        })
        ->rawColumns(['status','name','action']);
    }

    /**
     * Get query source of dataTable.
     *
     * @param \App\Models\Admin\SubModel $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(SubModel $model)
    {
        return $model->newQuery();
    }

    /**
     * Optional method if you want to use html builder.
     *
     * @return \Yajra\DataTables\Html\Builder
     */
    public function html()
    {
                return $this->builder()
                ->setTableId('sub-models-datatable-table')
                ->columns($this->getColumns())
                ->minifiedAjax()
                ->orderBy(1);
    }

    /**
     * Get columns.
     *
     * @return array
     */
    protected function getColumns()
    {
        return [
            Column::make('DT_RowIndex')->title(__('Sl No'))->searchable(false)->orderable(false),
            Column::make('name')->title(__('Name')),
            Column::make('brand_id')->title(__('Brand')),
            Column::make('difficulty_id')->title(__('Sales Difficulty')),
            Column::make('status')->title(__('Status'))->orderable(false),
            Column::computed('action')
                ->title(__('Action'))
                ->exportable(false)
                ->printable(false)
                ->width(60)
                ->addClass('text-center'),
        ];
    }

    /**
     * Get filename for export.
     *
     * @return string
     */
    protected function filename()
    {
        return 'Admin\SubModel_' . date('YmdHis');
    }
}
