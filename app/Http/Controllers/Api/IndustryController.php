...
class IndustryController extends Controller
{
    ...
    {
        $data['industry'] = Industry::where('uuid',$uuid)->where('status',1) ->with(['content' => function ($query) {
            $query->where('status', 1)->select('id','uuid','industries_id','title','description','order','image','button_title','button_link')->orderBy('order','ASC');
        }])->first();
