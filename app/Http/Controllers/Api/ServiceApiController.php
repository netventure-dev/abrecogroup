    ...
    public function index()
    {
        $data['services'] = Service::select('id', 'uuid', 'name','cover_image','logo','slug','cover_description','title','description','status')
                                ->with(['faqs'=> function($query) {
                                    ...
                                    $query->select('id','service_id','service as service_name', 'uuid', 'name','cover_image','logo','slug','cover_description','title','description');
                                    ->where('status',1);
    ...
    public function details($uuid)
    {
        $data['services'] = Service::select('id', 'uuid', 'name','cover_image','logo','slug','cover_description','title','description','status')
                                ->with(['faqs'=> function($query) {
                                    ...
                                    $query->select('id','service_id','service as service_name','service_slug', 'uuid', 'name','cover_image','logo','slug','cover_description','title','description');
                                    // ->where('status',1);
                                },'subservices.innerservices'=> function($query) {
