    ...
    public function details($uuid)
    {
        $data['services'] = Service::select('id', 'uuid', 'name','cover_image','logo','slug','cover_description','title','description','status')
                                ->with(['faqs'=> function($query) {
                                    ...
                                    $query->select('id','sub_service_id', 'service as service_name','service_slug','subservice as subservice_name', 'sub_service_slug','uuid', 'name','cover_image','logo','slug','cover_description','title','description')->where('status',1);
                                },])
                                ->where('uuid', $uuid)
                                ->where('status', 1)
                                ->orderBy('created_at', 'desc')
